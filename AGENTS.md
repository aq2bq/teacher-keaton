...
| **スキル利用時の生成ファイルは実行場所の `keaton/` に収める** | 形式モデル、解説書、一時ファイルが `/tmp`、`./tmp`、`./keaton` に分散すると、照合対象とGitで管理すべき範囲が利用ごとに変わるため。形式モデルは `keaton/spec/`、解説書は `keaton/explanation.md`、一時ファイルは `keaton/tmp/` とする。`examples/` の明示specパスは開発と回帰専用 |
