...
2. **判定依頼の列挙** — モデルが置いた主張を、判定可能な文で並べる:
   ...
   **測度の極性と閾値の意味**。各項目は「合っている/違う」で反応できる形にする。
   測度は必ず一項目立てる(「『<名前>』は<大きいほど悪い>で、<3>以上が<要対応>。
   合っていますか」)。極性は人間なら一瞬で判定でき、機械には導出できない
3. **フェーズ0の照合基準との突合** — 「計画で合意した『正しく理解できた』証拠はX。
   この成果物はそれを満たすか」を明示して判定を仰ぐ
4. **反応のしかたの案内** — 食い違いは「Xは実はY」の形で指摘してもらう。
   指摘があればモデルを直して再投影する
5. **未確定事項と対象外を別々に報告** — 未解決の食い違いは `TODO.md` のレベル2見出し、
   運用未確定は `about.operationalUndecided`、対象外は `about.exclusions` の要素を数え、
   件数と内容を別項目で示す。0件も0件と明記する
6. **(あれば)未解決項目の判断依頼** — 下記の `TODO.md` の項目
