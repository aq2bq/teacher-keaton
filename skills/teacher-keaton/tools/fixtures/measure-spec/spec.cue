...
glossary: {
	for _, x in states {
		(x.preferredName): {id: x.id, kind: "status", definition: x.definition}
	}
	for _, x in events {
		(x.preferredName): {id: x.id, kind: "event", definition: x.definition}
	}
	for _, m in measures {
		(m.preferredName): {id: m.id, kind: "measure", definition: m.definition}
		for _, t in m.thresholds {
			(t.preferredName): {id: t.id, kind: "threshold", definition: t.meaning}
		}
	}
}
